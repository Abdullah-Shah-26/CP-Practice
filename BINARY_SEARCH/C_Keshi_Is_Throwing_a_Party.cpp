...
bool canInvite(int target, vector<int>& maxRicher, vector<int>& maxPoorer, int n) {

    int invitedSoFar = 0;

    for (int i = 0; i < n; i++) {

        int poorer = invitedSoFar;
        int richer = target - invitedSoFar - 1;

        if (poorer <= maxPoorer[i] && richer <= maxRicher[i]) {
            invitedSoFar++;
        }
    }

    return invitedSoFar >= target;
}
