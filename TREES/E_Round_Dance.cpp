...
void dfs(int i, bool &isCycle, int &len){
  vis[i] = 1;
  len++;

  // If deg < 2 -> Not Pure Cycle
...
void solve(){
    ...
    for(int i = 0; i < n; i++){
      if(!vis[i]){
        ...
        if(isCycle && len > 2) cycleCase++;
        else bambooCase++;
      }
    }

    if(bambooCase){
      // MINI =  Cycles >= 3 + (All Merged Bamboos == 1) 
      // MAXI =  Cycles >= 3 + All Bamboos seperate
      cout << cycleCase + 1 << " " << cycleCase + bambooCase << endl;
    }
    else{
      cout << cycleCase << " " << cycleCase << endl;
    }
}
