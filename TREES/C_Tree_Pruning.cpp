...
// PostOrder DFS to compute maxDepth of Subtree of each node
void dfs(int u, int p){
  bool isLeaf = true; // Assuming I am leaf
  maxDepthSub[u] = depth[u]; // If leaf -> ans = it's own depth

  for(int v : adj[u]){
    if(v == p){
      continue;
    }
    // It has child , means its not a leaf
    isLeaf = false;
    dfs(v,u);
    maxDepthSub[u] = max(maxDepthSub[u], maxDepthSub[v]);
  }
}
...
void solve(){
    ...
    vi cnt(n + 2,0); // Stores nodes that survive when depth = D
