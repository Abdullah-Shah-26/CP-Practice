...
void solve(){
    ...
    for(auto x : a){
        ...
        ll chunkTime = t * z + y;
        ll fullChunks = T / chunkTime;
        ll balloons = fullChunks * z;

        ll remTime = T % chunkTime;
        ll extra = min(z, remTime / t);

        total += balloons + extra;
    ...
    for(int i = 0; i < n; i++){
      ...
      ll chunkTime = t * z + y;
      ll fullChunks = res/chunkTime;
      ll balloons = fullChunks * z;

      ll remTime = res % chunkTime;
      ll extra = min(z,remTime/t);

      ll totalBalloons = balloons + extra;

      ll take = min(totalBalloons, remaining);
