...
ll pow10[20];
vi digits;
// {count of distinct dig,sum}
pll dp[20][1024][2][2]; // pos,mask,tight,leadingZeros
bool vis[20][1024][2][2];
int k,n;

pll dfs(int i,int mask,bool tight,bool lz){
  if(i == n){
    return {lz ? 0 : 1, 0};
}

  if(vis[i][mask][tight][lz]){
    return dp[i][mask][tight][lz];
  }

  vis[i][mask][tight][lz] = 1; 

  int ub = tight==1 ? digits[i] : 9;
  int rem = n - i - 1;
  ...
  for(int d = 0; d <= ub; d++){
    ...
    auto child = dfs(i + 1, nextMask , newTight, nlz);
    ...
    if(!(lz && d == 0)){
      totalSum = (totalSum + ((d * place)%MOD * cnt)%MOD)%MOD;
    }
    totalCnt = (totalCnt + cnt) % MOD;
  }

 return dp[i][mask][tight][lz] = {totalCnt,totalSum};
}
