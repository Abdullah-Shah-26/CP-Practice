...
int query(int i, int j) {
  ...
  cout << "? " << i + 1 << ' ' << j + 1 << endl;
  int ans;
  cin >> ans;  // Reads a[i] * a[j] from the judge (or from input in hacked
               // version)
  return ans;
}

int common(pair<int, int> a, pair<int, int> b) {
  // Returns the common element between two pairs (they are known to share
  // exactly one)
...
void solve() {
  ...
  // Precompute all pairwise products -> (value1, value2) mapping for quick
  // reverse lookup
